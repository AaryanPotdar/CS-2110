...
int sort_name(void) {
  ...
  // bubble sort
  for (int i = (gradebook.size - 1); i > 0; i--) {
    for (int j = 0; j < i; j++) {
...
int print_gradebook(void) {
  ...
  for (int i = 0; i < gradebook.size; i++) {
    ...
    if (gradebook.entries[i].student.major == CS) {
      printf("CS,");
    } else if (gradebook.entries[i].student.major == CE) {
      printf("CE,");
    } else if (gradebook.entries[i].student.major == EE) {
      printf("EE,");
    } else {
      printf("IE,");
    }
    for (int a = 0; a < NUM_ASSIGNMENTS; a++) {
      printf("%.2f,", gradebook.entries[i].grades[a]);
    }
    printf("%.2f", gradebook.entries[i].average);
    printf("\n");
  }
  ...
  av++;
  printf("%.2f",gradebook.assignment_averages[av]);
  printf("\n");
